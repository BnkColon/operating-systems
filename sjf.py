...
# $ python sjf.py input.txt	

from string import *
from sys import argv

if (len(argv) == 2):		# If recive $ python sjf.py input.txt
	fileInput = argv[1] 	# Save the file input gived in the command line
							# print type(fileInput) / <type 'str'>
else:						# If not recieve this $ python sjf.py input.txt
...
for i in range(len(lists)):		# Go througth the list
	...
	processes.sort()			# Sort the processes list

for i in range(len(processes)):	# Go througth the sorted list
...
average = float(average) / len(processes)	# Calculate the average
print 'Avg:%1.2f' % (average)	# Print Avg : float final number with 1 decimal space